...
namespace std
{
  ...
  struct hash<std::unordered_set<int>>
  {
    int operator()(const std::unordered_set<int>& s) const
    {
      ...
      for (auto&& e : s)
      {
        setHash += pow(2, e); // check if this is implemeted with bitshift when powers of
                              // two, (likely isnt)
      }
...
void buildCycleList(std::vector<std::vector<bool>> G,
...
bool graphExtends(std::unordered_set<std::unordered_set<int>> cycleList, int vertices)
{
  for (auto&& c : cycleList)
  {
    if (
      ...
      !cycleExtends(
        c,
        cycleList)) // dont check hamiltonian cycles (TODO:optimize for n < v-1 and n > 4)
